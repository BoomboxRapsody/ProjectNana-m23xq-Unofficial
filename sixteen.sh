...
if [ "$#" -lt 6 ]; then
    echo "Usage: $0 <STOCK_DEVICE> <STOCK_DEVICE_CSC> <USE_UI_8_TETHERING_APEX> <TARGET_DEVICE> <TARGET_DEVICE_CSC> <OUTPUT_FILESYSTEM>"
    exit 1
fi
...
EXTRACT_SUPER "$FIRM_DIR/$STOCK_DEVICE"
EXTRACT_FIRMWARE "$FIRM_DIR/$STOCK_DEVICE" "all"
EXTRACT_SUPER "$FIRM_DIR/$TARGET_DEVICE"
EXTRACT_FIRMWARE "$FIRM_DIR/$TARGET_DEVICE" "all"

DECODE_OMC "$FIRM_DIR/$STOCK_DEVICE" "$WORK_DIR"
...
BUILD_PROP "$FIRM_DIR/$TARGET_DEVICE" "system" "ro.build.display.id" "${B_ID} ${B_V} V-${VERSION}: Built with Project Nana"
BUILD_PROP "$FIRM_DIR/$TARGET_DEVICE" "product" "ro.build.display.id" "${B_ID} ${B_V} V-${VERSION}: Built with Project Nana"
