...
SRC_MOUNT="${FIRM_TARGET_DIR}/${IMG_NAME_BASE}_mount"

FILE_CONTEXTS="${FIRM_TARGET_DIR}/config/${IMG_NAME_BASE}_file_contexts"
FS_CONFIG="${FIRM_TARGET_DIR}/config/${IMG_NAME_BASE}_fs_config"

# umount img if previously mounted
umount "$SRC_MOUNT" 2>/dev/null || true
rm -rf "$SRC_MOUNT"

rm -rf "${FIRM_TARGET_DIR}/$IMG_NAME_BASE"
mkdir "${FIRM_TARGET_DIR}/$IMG_NAME_BASE"

# config directory
mkdir -p "${FIRM_TARGET_DIR}/config"
mkdir -p "$SRC_MOUNT"
...
cp -ar "$SRC_MOUNT"/* "${FIRM_TARGET_DIR}/$IMG_NAME_BASE"
